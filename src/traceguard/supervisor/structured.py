...
from traceguard.types import Observation, PostRunAssessment, SandboxEvidence, SupervisorOutput, ToolCall
...
class GeminiSupervisor:
    ...
    def reevaluate(self, user_task: str, call: ToolCall, evidence: SandboxEvidence) -> PostRunAssessment:
        try:
            ...
            raise RuntimeError("install TraceGuard with the 'gemini' extra") from exc
        client = genai.Client(api_key=self.api_key)
        payload = {"user_task": user_task, "call": call.model_dump(mode="json"), "sandbox_evidence": evidence.model_dump(mode="json")}
        response = client.models.generate_content(
            model=self.model,
            contents=f"Reevaluate risk after a contained trial. Never assume sandbox success authorizes host execution.\n{json.dumps(payload)}",
            config={"response_mime_type": "application/json", "response_schema": PostRunAssessment, "temperature": 0},
        )
...
class OllamaSupervisor:
    ...
    def _generate(self, prompt: str, schema: dict[str, Any]) -> str:
        data = json.dumps({"model": self.model, "prompt": prompt, "stream": False, "format": schema, "options": {"temperature": 0}}).encode()
        request = urllib.request.Request(f"{self.base_url}/api/generate", data=data, headers={"Content-Type": "application/json"})
        with urllib.request.urlopen(request, timeout=self.timeout) as response:
            body = json.loads(response.read())
        return str(body["response"])

    def evaluate(self, user_task: str, call: ToolCall, observations: list[Observation]) -> SupervisorOutput:
        raw = self._generate(_request_payload(user_task, call, observations), SupervisorOutput.model_json_schema())
        return SupervisorOutput.model_validate_json(raw)

    def reevaluate(self, user_task: str, call: ToolCall, evidence: SandboxEvidence) -> PostRunAssessment:
        payload = {"user_task": user_task, "call": call.model_dump(mode="json"), "sandbox_evidence": evidence.model_dump(mode="json")}
        raw = self._generate(f"Reevaluate contained execution risk.\n{json.dumps(payload)}", PostRunAssessment.model_json_schema())
